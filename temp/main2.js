...
class makeTable {
 static makeTableRow(field, type) {
  ...
  makeBtnGroup.className = 'btn-group';
  ...
  makeXBtn.className = 'btn text-danger font-weight-bold f-tr-del-btn f-site-btn';
  ...
  makeCBtn.className = 'btn text-success f-tr-clone-btn f-site-btn';
 ...
 switch (type) {
  case 'text':
   ...
   break;
  case 'password':
   newField.placeholder = 'enter password';
   newField.setAttribute('name', 'password-name');
   break;
	case 'button':
...
function styleBtns() {
  for(var i =0 ; i < btns.length; i++) {
    ...
    // btns[i].classList.add('text-dark');
...
// show modal by edit btn in every row
 function startEdit(e) {
    ...
    fillEditStaticPanel();
    styleBtns();
	} else if(e.target.classList.contains('f-tr-clone-btn')) {
   ...
   newRow.lastElementChild.lastElementChild.lastElementChild.classList.remove('pr-5');
...
`;
    ...
    formOutputBtn.innerHTML = '<i class="fas fa-poll-h"></i> Get code/';
    }
  }
}
function copyCode() {
...
function addCustom() {
  ...
  newAttrRow.innerHTML = `<div class="input-group-prepend"><span class="input-group-text text-white bg-dark border-0">${newAttrName}:</span></div><input type="text" class="p-1 pl-3 form-control" name="${newAttrName}" value="${newAttrValue}"><div class="input-group-append"><span class="btn text-danger f-del-attr-btn bg-dark border-0" title="Delete '${newAttrName}' attribute">X</span>`;
